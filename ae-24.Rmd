...
- Due tomorrow
  - Project report first draft (in GitHub repo)
    - The other group will be given read access.
- Upcoming
...
Linear logistic regression (also simply called "logistic regression") takes in a number of predictors and outputs the probability of a success in a binary outcome variable. The probability is related to the predictors via a **sigmoid link function**,
$$
...
Choose a critical probability. This is called a **threshold probability** or **decision boundary** by which we decide to classify future observations.
...
`logistic_reg()`, engine `glm`, family `binomial`
...
- What is the log-odds? (Recall from the prep that log-odds $= \frac{p}{1-p}$)
...
manualLogOdds
# check work
logOdds = predict(fit_1$fit, newdata)
logOdds
```
...
- Change `decisionBoundary` in the code above to 0.0 and 1.0. Do the results surprise you? Why or why not?

## Classify a new email

```{r read-email}
email = readLines("test-email.txt")
email
...
- Do you believe the predicted log-odds of the email being spam? Why or why not?
